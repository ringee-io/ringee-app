...
export function OrgSwitcher({ useMock }: { useMock?: boolean }) {
  ...
  const [isLoadingSubscription, setIsLoadingSubscription] = React.useState(
    useMock ? false : true,
  );
  ...
  React.useEffect(() => {
    ...
    const checkSubscription = async () => {
      try {
        const response = await api.get<{ hasAvailable: boolean }>(
          ...
          >
            ...
            <div className="flex flex-col gap-0.5 leading-none">
              <span className="font-semibold">
                {tSwitcher("createNewOrganization")}
              </span>
              <span className="text-xs opacity-80">
                {tSwitcher("subscriptionReady")}
              </span>
            </div>
          </SidebarMenuButton>

          <Dialog
            open={isUpgradeCreateOpen}
            onOpenChange={setIsUpgradeCreateOpen}
          >
        ...
        <DropdownMenu>
            ...
            >
              ...
              <div className="flex flex-col gap-0.5 leading-none">
                <span className="font-semibold">
                  {isSwitchingOrg
                    ? tSwitcher("switching")
                    : activeOrganization?.name || tSwitcher("myPersonal")}
                </span>
                <span className="text-xs text-muted-foreground">
          ...
          >
            ...
            >
              ...
              {!activeOrganization && <Check className="ml-auto" />}
            </DropdownMenuItem>
            <DropdownMenuSeparator />
