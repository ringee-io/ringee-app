...
export class CampaignService {
  constructor(
    ...
    private readonly leadWrites: CampaignLeadWriteService,
  ) {}
  ...
  async addLeadsManually(
    ...
    await this.leadWrites.releaseLeadsIfRunning(campaignId);
   ...
   * contact fields. See {@link CampaignLeadWriteService.addContactToCampaign}
   * for the ownership, idempotency and queue-release rules it applies.
   */
  async addContactToCampaign(
    ...
    return this.leadWrites.addContactToCampaign(ctx, campaignId, contactId);
  }

  /**
   * Ownership gate for writing leads into a campaign. See
   * {@link CampaignLeadWriteService.assertCampaignForLeadWrite}.
   */
  async assertCampaignForLeadWrite(
    ctx: OwnershipContext,
    campaignId: string,
  ): Promise<Campaign> {
    return this.leadWrites.assertCampaignForLeadWrite(ctx, campaignId);
  }
