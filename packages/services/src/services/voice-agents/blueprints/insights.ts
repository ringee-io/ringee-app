...
function outcomeInsight(
  ...
  return {
    ...
    instructions: [
      ...
      guidance,
      `Use "${AiVoiceAgentOutcome.no_answer}" when the person never answered or`,
      "the call reached voicemail. Use",
      `"${AiVoiceAgentOutcome.no_conversation}" only when the call was answered`,
      "but the person did not meaningfully speak before hanging up. Use",
      `"${AiVoiceAgentOutcome.wrong_number}" when the person explicitly said the`,
      "number or intended recipient was wrong. Use",
