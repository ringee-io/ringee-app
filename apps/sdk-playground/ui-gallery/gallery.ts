...
function buildFloatingSection(): HTMLElement {
  return section("Floating dialer", [
    ...
    panelTile("2 · Email", "Identify your account", { auth: "anonymous", state: "uninitialized" }),
    panelTile("3 · Code (OTP)", "6 digits with auto-advance", {
      auth: "awaiting_code", state: "uninitialized",
      patch: (m) => { m.challenge = { id: "c", maskedEmail: "ta***@company.com", expiresAt: new Date(Date.now() + 3e5), resendAvailableAt: new Date(Date.now() + 12000) }; },
    }),
    ...
    panelTile("8 · Error", "Incorrect code with recovery", {
      auth: "awaiting_code", state: "uninitialized",
      patch: (m) => {
        m.challenge = { id: "c", maskedEmail: "ta***@company.com", expiresAt: new Date(Date.now() + 3e5), resendAvailableAt: new Date(Date.now() + 12000) };
...
function buildBarSection(): HTMLElement {
  return section("Dialer Bar", [
    barTile("10 · Email", "Inline sign-in", { auth: "anonymous", state: "uninitialized" }),
    ...
    barTile("14 · Narrow container (320px)", "Collapses to essentials", {
      auth: "authenticated", state: "active",
      patch: (m) => { m.contact = CONTACT; m.number = CONTACT.number; m.activeCall = activeCall(true, "active"); },
    }, 320),
  ]);
...
function buildLiveSection(): HTMLElement {
  ...
  wrap.append(
    (() => { const p = el("p", "hint"); p.textContent = "Interactive demo (simulated, no backend). Enter any email → any 6-digit code → Call."; return p; })(),
    barFrame,
    (() => { const p = el("p", "hint"); p.textContent = "The Floating dialer is in the bottom-right corner ↘"; return p; })(),
  );
  const sec = el("section", "");
  ...
  queueMicrotask(() => {
    createBar({ dialer: new MockDialer({ mode: "interactive" }) as never, container: barHost, locale: "en", allowHold: true, theme });
    createFloating({ dialer: new MockDialer({ mode: "interactive" }) as never, locale: "en", allowHold: true, side: "right", theme });
  });
  return sec;
}
...
header.append(
  ...
  (() => { const p = el("p", "sub", "Floating & Bar · shared visual system · complete states"); return p; })(),
  themeToggle(),
);
