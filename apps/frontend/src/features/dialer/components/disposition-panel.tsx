...
export function DispositionPanel({ campaignId, sessionId }: Props) {
  ...
  const dispositionRequired = useDialerAttemptStore((s) => s.dispositionRequired);
  const availableDispositions = useDialerAttemptStore((s) => s.availableDispositions);
  ...
  const selectedDispo = availableDispositions.find((d) => d.code === selectedCode);
  ...
  async function handleSubmit() {
    ...
    try {
      ...
      if (showCallbackFields && callbackDate) {
        body.callbackScheduledAt = callbackDate;
