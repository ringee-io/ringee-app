...
import AgenticMode from './agentic-mode';
...
const Hero = () => {
  ...
  const capabilityPills = [
    {
      icon: IconDialpad,
      label: t('capabilities.manualDialer'),
      color: 'text-cyan-500'
    },
    {
      icon: IconEye,
      label: t('capabilities.previewDialer'),
      color: 'text-violet-500'
    },
    {
      icon: IconBolt,
      label: t('capabilities.progressiveDialer'),
      color: 'text-amber-500'
    },
    {
      icon: IconPlugConnected,
      label: t('capabilities.crmSync'),
      color: 'text-emerald-500'
    },
    {
      icon: IconPlayerRecord,
      label: t('capabilities.recordings'),
      color: 'text-rose-500'
    },
    {
      icon: IconDeviceMobile,
      label: t('capabilities.browserMobile'),
      color: 'text-sky-500'
    }
  ];
  ...
  return (
    <section className='relative isolate flex flex-col items-center overflow-hidden px-6 pt-16 pb-6'>
      ...
      {/* Mode toggle — classic hero ⇆ agentic flow */}
      <motion.div
        initial={{ opacity: 0, y: -6 }}
        animate={{ opacity: 1, y: 0 }}
        transition={{ delay: 0.15 }}
        className='border-border/60 bg-background/70 relative z-10 mb-8 inline-flex items-center rounded-full border p-1 shadow-sm backdrop-blur-sm'
      >
        {(['classic', 'agentic'] as const).map((m) => {
          const active = mode === m;
          return (
            <button
              key={m}
              onClick={() => setMode(m)}
              aria-pressed={active}
              className={`relative inline-flex items-center gap-1.5 rounded-full px-4 py-1.5 text-sm font-semibold transition-colors ${
                active
                  ? 'text-white'
            ...
            >
              {active && (
                <motion.span
                  ...
                  className='absolute inset-0 -z-10 rounded-full bg-gradient-to-r from-amber-500 via-violet-500 to-cyan-500'
