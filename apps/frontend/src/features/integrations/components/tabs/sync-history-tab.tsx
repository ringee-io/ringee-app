      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-xs text-muted-foreground">
            Last 50 sync attempts. Failed or ambiguous syncs can be retried or resolved.
          </p>
        </div>
        <Button
          variant="ghost"
      ...
      {loading ? (
        ...
        <div className="flex h-48 flex-col items-center justify-center text-center">
          ...
          <p className="mt-1 max-w-xs text-xs text-muted-foreground">
            Call syncs will appear here once your first call is logged to the CRM.
          </p>
        </div>
      ) : (
        <div className="space-y-2">
        ...
        <div className="flex items-start gap-2">
          ...
          <div className="min-w-0">
            ...
            <p className="mt-1 font-mono text-[11px] text-muted-foreground truncate">
              call {sync.callId.slice(0, 8)}… · {new Date(sync.updatedAt).toLocaleString()}
            </p>
...
function ResolveMatchDialog({
  ...
  const [externalType, setExternalType] = useState<'person' | 'company'>('person');
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent>
        <DialogHeader>
          ...
          <DialogDescription>
            Multiple CRM records matched this call. Provide the external ID of the
            correct record to link this call to.
          </DialogDescription>
        </DialogHeader>
        ...
        <div className="space-y-4">
          {sync && (
            <div className="rounded-md bg-muted/50 p-3">
              <p className="text-xs text-muted-foreground">
                Call ID: <span className="font-mono">{sync.callId.slice(0, 12)}…</span>
              </p>
              {sync.lastError && (
        ...
        <DialogFooter>
          ...
          <Button onClick={handleResolve} disabled={!externalId.trim() || loading}>
