...
export function CustomIntegrationCreateDialog({ open, onOpenChange, onCreated }: Props) {
  const { create } = useCustomIntegrations();
  const [name, setName] = useState('');
  const [creating, setCreating] = useState(false);
  const [result, setResult] = useState<CustomIntegrationWithSecrets | null>(null);
        ...
        {!result ? (
          <>
            <DialogHeader>
              ...
              <DialogDescription>
                Connect Ringee to any external system using an API key, webhooks and click-to-call.
              </DialogDescription>
            </DialogHeader>
    ...
    <>
      <DialogHeader>
        ...
        <DialogDescription className="flex gap-1.5">
          ...
          <span>
            The API key and webhook signing secret will <strong>only be shown once</strong>. Copy them
            now and store them in a secure location. You can rotate them later, but you cannot view
            them again.
          </span>
        </DialogDescription>
      </DialogHeader>
      <div className="space-y-4 py-2">
        ...
        <CopyableSecret label="Webhook signing secret" value={item.signingSecret ?? ''} />
      </div>
      <DialogFooter>
