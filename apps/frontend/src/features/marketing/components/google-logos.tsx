/**
 * Google brand marks, inlined so they carry their own colours in both themes —
 * unlike the agent logomarks, which inherit `currentColor`. Meet is the only
 * one the marketing pages need: meetings are booked to Google Calendar and the
 * invite carries a Meet link.
 *
 * Used under nominative fair use to identify the integration.
 */

export function GoogleMeetLogo({ className }: { className?: string }) {
  return (
    <svg viewBox='0 0 87.5 72' className={className} aria-hidden>
      <path
        fill='#00832d'
