      ...
      <DropdownMenuContent align='end' className='w-72'>
        {(['KPI', 'Chart', 'Table'] as const).map((cat, idx) => (
          <React.Fragment key={cat}>
            ...
            <DropdownMenuLabel>{t(`categoryLabel.${cat}`)}</DropdownMenuLabel>
            {grouped[cat].map((w) => {
              const already = existingTypes.has(w.type);
              const title = t(`catalog.${w.type}.title`);
              const description = t(`catalog.${w.type}.description`);
              return (
                <DropdownMenuItem
                  key={w.type}
                  disabled={already}
                  onSelect={() =>
                    onAdd(w.type, title, w.defaultSize.w, w.defaultSize.h)
                  }
                >
