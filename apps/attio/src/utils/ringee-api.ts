...
export async function getRingeeApiUrl(): Promise<string> {
    ...
    return (typeof url === "string" && url.length > 0 ? url : DEFAULT_RINGEE_API_URL).replace(
        /\/$/,
        ""
    )
}
...
export async function ringeeRequest<T>(
    ...
    const [apiUrl, token] = await Promise.all([getRingeeApiUrl(), getRingeeToken()])
    ...
    if (!response.ok) {
        ...
        console.error(`[Ringee API] ${options.method ?? "GET"} ${path} failed: ${response.status}`)

        if (response.status === 401) {
            throw new Error("Authentication failed. Please check your Ringee integration token.")
        }
