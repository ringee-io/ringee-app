        ...
        return (
            <Section title={name}>
                <Banner variant="warning">
                    No phone numbers found for this person. Add a phone number to their Attio record
                    to call them.
                </Banner>
            </Section>
        )
...
function CompanyCallDialog({recordId, object}: {recordId: string; object: string}) {
    ...
    if (uniquePhones.length === 0) {
        return (
            <Section title={name}>
                <Banner variant="warning">
                    No phone numbers found for team members of this company. Add phone numbers to
                    team members in Attio to call them.
                </Banner>
            </Section>
        )
    }
...
function PhoneListWithCallerIds({
    ...
    const [callResult, setCallResult] = useState<{dialerUrl: string} | null>(null)
    ...
    async function initiateCall(toNumber: string) {
        if (!defaultCallerId) {
            setError("No caller ID available. Configure a phone number in Ringee first.")
            return
        }
        ...
        try {
            ...
            const message = err instanceof Error ? err.message : "Failed to prepare call"
            setError(message)
            setCalling(false)
        }
    }
