...
function ConnectionStatus() {
    ...
    const hasToken = typeof settings.ringee_token === "string" && settings.ringee_token.length > 0
...
function ConnectionValidator() {
    ...
    if (validation.valid) {
        return (
            <Section title="Connection Status">
                ...
                <Typography.Body>User: {validation.context.userName ?? "Unknown"}</Typography.Body>
                <Typography.Body>
                    Scope:{" "}
                    <Badge color={validation.context.scope === "organization" ? "blue" : "grey"}>
    ...
    return <Banner variant="error">Connection validation failed. Please check your token.</Banner>
}
        ...
        <Form>
            <FormSection title="Ringee Connection">
                <TextInput
                    ...
                    maxLength={2000}
                />
                <TextInput label="Ringee API URL" name="ringee_api_url" maxLength={500} />
            </FormSection>
        </Form>
    )
}
