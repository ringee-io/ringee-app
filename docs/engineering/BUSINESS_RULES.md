...
simultaneous meetings; the slot remains bookable until that capacity is reached.
Without saved windows, the backwards-compatible 09:00–18:00, capacity-one
schedule applies. Google and Microsoft are optional outbound sync targets and
never a prerequisite for activating or booking with an appointment agent.

- **Risk if violated:** the agent exceeds the configured group capacity or tells
  the person a time that was never available
